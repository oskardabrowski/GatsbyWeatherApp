...
const Sliders = () => {
  ...
  useEffect(() => {
      const slider = document.querySelector(".SliderInitialList");
      const sliders = document.querySelector(".SliderInitialList").querySelectorAll('li');
      sliders.forEach((el) => el.style.opacity = 0);
      const newSlides = state.map((el, index) => {return <li key={index}><img src={el} alt={el} /></li>})

      slider.innerHTML = newSlides;
    //   setTimeout(() => {
